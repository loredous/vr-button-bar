...
vr::EVRInitError MyDeviceProvider::Init( vr::IVRDriverContext *pDriverContext )
{
	...
	my_buttonbar_ = std::make_unique< MyControllerDeviceDriver >(vr::TrackedControllerRole_OptOut);
	...
	if ( !vr::VRServerDriverHost()->TrackedDeviceAdded( my_buttonbar_->MyGetSerialNumber().c_str(), vr::TrackedDeviceClass_Controller, my_buttonbar_.get() ) )
	{
		DriverLog( "Failed to create controller device!" );
...
void MyDeviceProvider::RunFrame()
{
	...
	{
		if ( my_buttonbar_ != nullptr )
		{
			my_buttonbar_->MyProcessEvent( vrevent );
			my_buttonbar_->MyRunFrame();
		}
	}
}
